...
/**
 * 
 * This class is the registry server, it stores the information of where the
 * service is. It is used by the dispatch server to add new service and the
 * client to lookup service.
 * 
...
public class RegistryServer {
	/**
	 * The table to store remoteObjectReference 1st map: Key: serviceName,
	 * Value: 2nd map 2nd map: Key: server running this service(may exist
	 * several servers), Value: ObjectKey
	 */
	...
	private static final String HOST_NAME = "128.237.217.63";
	 ...
	 * 
	 * @param serviceName
	 *            the service name to be added
	 * @param ip
	 *            dispatch server's ip
	 * @param port
	 *            dispatch server's listening port
	 * @return message to the dispatch server to tell if the request is
	 *         successful
	 */
	public CommunicationMessage addService(String serviceName, String ip,
			int port, long objectKey) {
		// new serviceName
		...
		if (!remoteObjectTable.containsKey(serviceName)) {
			...
			replyMessage = new CommunicationMessage(MessageType.ReplyToServer,
			 ...
			 System.out.println("Add service " +  serviceName + " successfully!");
			...
			if (services.containsKey(nodeID) && objectKey == -1) {
				replyMessage = new CommunicationMessage(
				 ...
				 System.out.println("Add " + serviceName + " failed: duplicate service on the same server");
			} else {
				services = new ConcurrentHashMap<NodeID, Long>();
				if (objectKey != -1) { // already new a object on dispatch
										// server and return the object key
					services.put(nodeID, objectKey);
				} else {
	...
	/**
	 * This method is called by client to look up a service. If there are
	 * several service running on different dispatch node, we randomly choose
	 * one dispatch node and return the RemoteObjectReference.
	 * 
	 * @param serviceName
	 *            the service name to look up
	 * @return null - if the service does not exist. ROR - if the service is
	 *         found
	 */
	public RemoteObjectReference lookup(String serviceName) {
		...
		if (!remoteObjectTable.containsKey(serviceName)) { // no such service
			...
			ror = new RemoteObjectReference(nodeID.getHostName(),
					nodeID.getPort(), serviceName);
